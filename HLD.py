...
from HLDapi.HLDnoderpc import HLDNodeRPC
from HLDbase.account import PrivateKey, PublicKey
from HLDbase import transactions, operations
...
class HLD(object):
    """ Connect to the HLD network.
        ...
        * **Wallet Database**: Here, the HLDlibs load the keys from the
          locally stored wallet SQLite database (see ``storage.py``).
          To use this mode, simply call ``HLD()`` without the
          ``keys`` parameter
        * **Providing Keys**: Here, you can provide the keys for
          your accounts manually. All you need to do is add the wif
          keys for the accounts you want to use as a simple array
          using the ``keys`` parameter to ``HLD()``.
    ...
    def connect(self,
                ...
                rpcpassword="",
                **kwargs):
        """ Connect to HLD network (internal use only)
        """
        if not node:
            if "node" in config:
                node = config["node"]
            else:
                raise ValueError("A HLD node needs to be provided!")
        ...
        self.rpc = HLDNodeRPC(node, rpcuser, rpcpassword, **kwargs)
    ...
    def finalizeOp(self, ops, account, permission, **kwargs):
        """ This method obtains the required private keys if present in
            ...
            :param object append_to: This allows to provide an instance of
                ProposalsBuilder (see :func:`HLD.new_proposal`) or
                TransactionBuilder (see :func:`HLD.new_tx()`) to specify
            ...
            ... note:: This uses ``HLD.txbuffer`` as instance of
                :class:`HLD.transactionbuilder.TransactionBuilder`.
                You may want to use your own txbuffer
        """
        if "append_to" in kwargs and kwargs["append_to"]:
            if self.proposer:
                log.warn(
                    "You may not use append_to and HLD.proposer at "
                    "the same time. Append HLD.new_proposal(..) instead"
                )
    ...
    def sign(self, tx=None, wifs=[]):
        ...
        """
        if tx:
            txbuffer = TransactionBuilder(tx, HLD_instance=self)
        else:
    ...
    def broadcast(self, tx=None):
        """ Broadcast a transaction to the HLD network
    ...
    def newWallet(self, pwd):
        """ Create a new wallet. This method is basically only calls
            :func:`HLD.wallet.create`.

            :param str pwd: Password to use for the new wallet
            :raises HLD.exceptions.WalletExists: if there is already a
                wallet created
        """
    ...
    def transfer(self, to, amount, asset, memo="", account=None, **kwargs):
        ...
        account = Account(account, HLD_instance=self)
        amount = Amount(amount, asset, HLD_instance=self)
        to = Account(to, HLD_instance=self)
    ...
    def create_account(
        ...
        **kwargs
        ...
        """ Create new account on HLD

            The brainkey/password can be used to recover all generated keys
            (see `HLDbase.account` for more details.
        ...
        try:
            Account(account_name, HLD_instance=self)
            raise AccountExistsException
        except:
            pass

        referrer = Account(referrer, HLD_instance=self)
        registrar = Account(registrar, HLD_instance=self)

        " Generate new keys from password"
        from HLDbase.account import PasswordKey, PublicKey
        if password:
        ...
        for k in additional_owner_accounts:
            addaccount = Account(k, HLD_instance=self)
        ...
        voting_account = Account(
            proxy_account or "proxy-to-self", HLD_instance=self)
    ...
    def allow(
        ...
        try:
            ...
            try:
                foreign_account = Account(foreign, HLD_instance=self)
    ...
    def approvewitness(self, witnesses, account=None, **kwargs):
        ...
        for witness in witnesses:
            witness = Witness(witness, HLD_instance=self)
    ...
    def approvecommittee(self, committees, account=None, **kwargs):
        ...
        for committee in committees:
            committee = Committee(committee, HLD_instance=self)
    ...
    def approveproposal(
        ...
        if not approver and not is_key:
            ...
            approver = Account(approver, HLD_instance=self)
        else:
        ...
        for proposal_id in proposal_ids:
            proposal = Proposal(proposal_id, HLD_instance=self)
            update_dict = {
                "fee": {"amount": 0, "asset_id": "1.3.0"},
    ...
    def approveworker(self, workers, account=None, **kwargs):
        ...
        for worker in workers:
            worker = Worker(worker, HLD_instance=self)
    ...
    def cancel(self, orderNumbers, account=None, **kwargs):
        ...
        if not account:
            raise ValueError("You need to provide an account")
        account = Account(account, full=False, HLD_instance=self)
    ...
    def vesting_balance_withdraw(self, vesting_id, amount=None, account=None, **kwargs):
        """ Withdraw vesting balance
            ...
            :param HLD.amount.Amount Amount: to withdraw ("all" if not provided")
            :param str account: (optional) the account to allow access
                to (defaults to ``default_account``)
        ...
        if not amount:
            obj = Vesting(vesting_id, HLD_instance=self)
    ...
    def publish_price_feed(
        ...
        """ Publish a price feed for a market-pegged asset
            ...
            :param HLD.price.Price settlement_price: Price for settlement
            :param HLD.price.Price cer: Core exchange Rate (default ``settlement_price + 5%``)
        ...
        assert mcr > 100
        assert mssr > 100
        assert isinstance(settlement_price, Price), "settlement_price needs to be instance of `HLD.price.Price`!"
        assert cer is None or isinstance(cer, Price), "cer needs to be instance of `HLD.price.Price`!"
        if not account:
            if "default_account" in config:
                account = config["default_account"]
        if not account:
            raise ValueError("You need to provide an account")
        account = Account(account, HLD_instance=self)
        asset = Asset(symbol, HLD_instance=self, full=True)
    ...
    def reserve(self, amount, account=None, **kwargs):
        """ Reserve/Burn an amount of this shares
            ...
            :param HLD.amount.Amount amount: The amount to be burned.
            :param str account: (optional) the account to allow access
                to (defaults to ``default_account``)
        """
        ...
        **kwargs
        ...
        """ Reserve/Burn an amount of this shares
            ...
            **Required**
            ...
            :param HLD.amount.Amount daily_pay: The amount to be paid daily
            ...
            **Optional**
            ...
            :param str account: (optional) the account to allow access
                to (defaults to ``default_account``)
        """
        from HLDbase.transactions import timeformat
    ...
    def fund_fee_pool(self, symbol, amount, account=None, **kwargs):
        ...
        if not account:
            raise ValueError("You need to provide an account")
        account = Account(account, HLD_instance=self)
        asset = Asset(symbol, HLD_instance=self)
