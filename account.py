...
class Account(BlockchainObject):
    """ This class allows to easily access Account data
        ...
        :rtype: dictionary
        :raises HLD.exceptions.AccountDoesNotExistsException: if account
                does not exist
        ...
        .. code-block:: python
            ...
            from HLD.account import Account
    ...
    def refresh(self):
        ...
        if re.match("^1\.2\.[0-9]*$", self.identifier):
            account = self.HLD.rpc.get_objects([self.identifier])[0]
        else:
            account = self.HLD.rpc.lookup_account_names(
                [self.identifier])[0]
        if not account:
        ...
        if self.full:
            account = self.HLD.rpc.get_full_accounts(
    ...
    def balances(self):
        """ List balances of an account. This call returns instances of
            :class:`HLD.amount.Amount`.
        """
        from .amount import Amount
        balances = self.HLD.rpc.get_account_balances(self["id"], [])
        return [
            Amount(b, HLD_instance=self.HLD)
    ...
    def call_positions(self):
        """ Alias for :func:HLD.account.Account.callpositions
        """
    ...
    def history(
        ...
        """ Returns a generator for individual account transactions. The
            ...
            :param array exclude_ops: Exclude thse operations from
                generator (*optional*)
        """
        from HLDbase.operations import getOperationNameForId
        _limit = 100
        cnt = 0

        mostrecent = self.HLD.rpc.get_account_history(
            self["id"],
            "1.11.{}".format(0),
            1,
        ...
        while True:
            # RPC call
            txs = self.HLD.rpc.get_account_history(
                self["id"],
                "1.11.{}".format(last),
                _limit,
                "1.11.{}".format(first - 1),
    ...
    def upgrade(self):
        return self.HLD.upgrade_account(account=self)


class AccountUpdate(dict):
    """ This purpose of this class is to keep track of account updates
        as they are pushed through by :class:`HLD.notify.Notify`.
    ...
    def account(self):
        """ In oder to obtain the actual
            :class:`HLD.account.Account` from this class, you can
