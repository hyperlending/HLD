...
from HLDbase import operations
...
class Market(dict):
    """ This class allows to easily access Markets on the blockchain for trading, etc.
        ...
        :param HLD.asset.Asset base: Base asset
        :param HLD.asset.Asset quote: Quote asset
        ...
        * ``base`` and ``quote`` are valid assets (according to :class:`HLD.asset.Asset`)
        ...
        * ``base-quote`` separated with ``-``
        ...
        .. note:: Throughout this library, the ``quote`` symbol will be
                  ...
                  :func:`HLD.market.Market.sell` or
                  :func:`HLD.market.Market.buy`, you will sell/buy **only
    ...
    def ticker(self):
        ...
        if "bitasset_data_id" in self["quote"]:
            bitasset = self.HLD.rpc.get_object(self["quote"]["bitasset_data_id"])
            ...
            bitasset = self.HLD.rpc.get_object(self["base"]["bitasset_data_id"])
            backing_asset_id = bitasset["options"]["short_backing_asset"]
        ...
        ticker = self.HLD.rpc.get_ticker(
            self["base"]["id"],
            self["quote"]["id"],
        )
        data["baseVolume"] = Amount(ticker["base_volume"], self["base"], HLD_instance=self.HLD)
        data["quoteVolume"] = Amount(ticker["quote_volume"], self["quote"], HLD_instance=self.HLD)
    ...
    def volume24h(self):
        ...
        """
        volume = self.HLD.rpc.get_24_volume(
            self["base"]["id"],
            self["quote"]["id"],
        )
        return {
            self["base"]["symbol"]: Amount(volume["base_volume"], self["base"], HLD_instance=self.HLD),
            self["quote"]["symbol"]: Amount(volume["quote_volume"], self["quote"], HLD_instance=self.HLD)
        }
    ...
    def orderbook(self, limit=25):
        """ Returns the order book for a given market. You may also
            ...
            .. note:: Each bid is an instance of
                class:`HLD.price.Order` and thus carries the keys
                ``base``, ``quote`` and ``price``. From those you can
                obtain the actual amounts for sale

        """
        orders = self.HLD.rpc.get_order_book(
            self["base"]["id"],
            self["quote"]["id"],
            limit
        )
        asks = list(map(lambda x: Order(
            Amount(x["quote"], self["quote"], HLD_instance=self.HLD),
            Amount(x["base"], self["base"], HLD_instance=self.HLD),
            HLD_instance=self.HLD
        ), orders["asks"]))
    ...
    def trades(self, limit=25, start=None, stop=None):
        ...
        orders = self.HLD.rpc.get_trade_history(
            self["base"]["symbol"],
            self["quote"]["symbol"],
            formatTime(stop),
            formatTime(start),
        ...
        return list(map(
            lambda x: FilledOrder(
                x,
                quote=Amount(x["amount"], self["quote"], HLD_instance=self.HLD),
                base=Amount(float(x["amount"]) * float(x["price"]), self["base"], HLD_instance=self.HLD),
                HLD_instance=self.HLD
            ), orders
        ))
    ...
    def accounttrades(self, account=None, limit=25):
        ...
        """
        if not account:
            if "default_account" in self.HLD.config:
                account = self.HLD.config["default_account"]
        if not account:
            raise ValueError("You need to provide an account")
        account = Account(account, HLD_instance=self.HLD)

        filled = self.HLD.rpc.get_fill_order_history(
            self["base"]["id"],
            self["quote"]["id"],
            2 * limit,
            api="history"
    ...
    def accountopenorders(self, account=None):
        """ Returns open Orders
            ...
            :param HLD.account.Account account: Account name or instance of Account to show orders for in this market
        """
        if not account:
            if "default_account" in self.HLD.config:
                account = self.HLD.config["default_account"]
        if not account:
            raise ValueError("You need to provide an account")
        account = Account(account, full=True, HLD_instance=self.HLD)
    ...
    def buy(
        ...
        """
        if not expiration:
            expiration = self.HLD.config["order-expiration"]
        if not account:
            if "default_account" in self.HLD.config:
                account = self.HLD.config["default_account"]
        if not account:
        ...
        if isinstance(amount, Amount):
            amount = Amount(amount, HLD_instance=self.HLD)
            assert(amount["asset"]["symbol"] == self["quote"]["symbol"]), \
                "Price: {} does not match amount: {}".format(
                    str(price), str(amount))
        else:
            amount = Amount(amount, self["quote"]["symbol"], HLD_instance=self.HLD)
        ...
        if returnOrderId:
            ...
            prevblocking = self.HLD.blocking
            self.HLD.blocking = returnOrderId

        tx = self.HLD.finalizeOp(order, account["name"], "active")

        if returnOrderId:
            tx["orderid"] = tx["operation_results"][0][1]
            self.HLD.blocking = prevblocking
    ...
    def cancel(self, orderNumber, account=None):
        ...
        """
        return self.HLD.cancel(orderNumber, account=account)
