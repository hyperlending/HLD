...
from HLDbase.objects import Operation
from HLDbase.account import PrivateKey, PublicKey
from HLDbase.signedtransactions import Signed_Transaction
...
class ProposalBuilder:
    """ Proposal Builder allows us to construct an independent Proposal
        ...
        :param int proposal_review: Number of seconds for review of the
            proposal
        :param HLD.transactionbuilder.TransactionBuilder: Specify
...
class TransactionBuilder(dict):
    ...
    def appendSigner(self, account, permission):
        ...
        def fetchkeys(account, perm, level=0):
            ...
            for authority in account[perm]["key_auths"]:
                try:
                    wif = self.HLD.wallet.getPrivateKeyForPublicKey(
                        authority[0])
            ...
            if sum([x[1] for x in r]) < required_treshold:
                ...
                for authority in account[perm]["account_auths"]:
                    auth_account = Account(
                        authority[0], HLD_instance=self.HLD)
        ...
        if account not in self.signing_accounts:
            ...
            if isinstance(account, PublicKey):
                self.wifs.add(
                    self.HLD.wallet.getPrivateKeyForPublicKey(
                        str(account)
                    )
                )
            else:
    ...
    def constructTx(self):
        ...
        ops = transactions.addRequiredFees(self.HLD.rpc, ops)
        expiration = transactions.formatTimeFromNow(self.HLD.expiration)
    ...
    def sign(self):
        ...
        if self.HLD.rpc:
            operations.default_prefix = (
                self.HLD.rpc.chain_params["prefix"])
        ...
        signedtx.sign(self.wifs, chain=self.HLD.rpc.chain_params)
    ...
    def verify_authority(self):
        ...
        """
        try:
            if not self.HLD.rpc.verify_authority(self.json()):
    ...
    def broadcast(self):
        ...
        # Broadcast
        try:
            if self.HLD.blocking:
                ret = self.HLD.rpc.broadcast_transaction_synchronous(
                    ret, api="network_broadcast")
                ret.update(**ret["trx"])
            else:
                self.HLD.rpc.broadcast_transaction(
                    ret, api="network_broadcast")
        except Exception as e:
            raise e
    ...
    def addSigningInformation(self, account, permission):
        ...
        """
        self.constructTx()
        self["blockchain"] = self.HLD.rpc.chain_params
    ...
    def appendMissingSignatures(self):
        ...
        for pub in missing_signatures:
            wif = self.HLD.wallet.getPrivateKeyForPublicKey(pub)
            if wif:
                self.appendWif(wif)
