...
from HLDbase.account import PrivateKey, GPHPrivateKey
...
class Wallet():
    """ The wallet is meant to maintain access to private keys for
        ...
        :param HLDNodeRPC rpc: RPC connection to a HLD node
        :param array,dict,string keys: Predefine the wif keys to shortcut the
               wallet database

        Three wallet operation modes are possible:

        * **Wallet Database**: Here, pyHLD loads the keys from the
    ...
    def setKeys(self, loadkeys):
        """ This method is strictly only for in memory keys that are
            passed to Wallet/HLD with the ``keys`` argument
        """
        log.debug(
    ...
    def getAllAccounts(self, pub):
        ...
        for id in self.getAccountsFromPublicKey(pub):
            try:
                account = Account(id)   # FIXME: self.HLD is not available in wallet!
            except:
                continue
    ...
    def getAccount(self, pub):
        ...
        if not name:
            ...
            try:
                account = Account(name)   # FIXME: self.HLD is not available in wallet!
            except:
                return
